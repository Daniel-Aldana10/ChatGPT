...
public class ChatGptAdapterProxy implements IAiAdapter {
    ...
    public ChatGptAdapterProxy(IAiAdapter realAdapter) {
        this(realAdapter, 5 * 60 * 1000, "[OpenAI no disponible, intente más tarde]"); // TTL 5 minutos por defecto
    }
    ...
    public String generateResponse(String input) {
        ...
        CachedResponse cached = cache.get(input);
        if (cached != null && (now - cached.timestamp) < ttlMillis) {
            System.out.println("[Proxy] Respuesta cacheada para: " + input);
        ...
        try {
            ...
            System.out.println("[Proxy] Llamada a OpenAI para: '" + input + "' en " + elapsed + "ms");
            cache.put(input, new CachedResponse(response, now));
            return response;
        } catch (Exception e) {
